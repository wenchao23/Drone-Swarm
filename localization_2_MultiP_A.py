...
def localization(Quad,dis,Sigma,Leader):
    ...
    for i in np.arange(start = 0, stop = n, step = 1):
        ...
        Quad[i].mu = np.array([Quad[i].X + np.random.randint(-1000, 1000), Quad[i].Y + np.random.randint(-1000, 1000)])
        Quad[i].mu_x = np.zeros(300)
        Quad[i].mu_y = np.zeros(300)
        Quad[i].Sigma = 1
    ...
    #dis = dis1
    for iter_all in np.arange(start=0,stop=300,step=1):
...
def init_pos(n,height):
    ...
    for i in np.arange(start=0,stop=n,step=1):
        randint1 = np.random.randint(-5000, 5000, size = (1, 2))
